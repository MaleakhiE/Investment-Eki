    ...
    <main className="app-page lg:ml-64 p-4 lg:p-6">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,.8fr)]">
        <section className="min-w-0 space-y-4">
          ...
          {error ? <div role="alert" className="card rounded-3xl border border-red-200 bg-red-50 p-6 text-sm text-red-700"><p className="font-medium">Account data is unavailable</p><p className="mt-1">{error}</p><button type="button" aria-label="Retry loading accounts" onClick={() => { setLoading(true); void loadAccounts(); }} className="mt-3 rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-red-700 underline">Retry</button></div> : loading ? <div className="card rounded-3xl p-6 text-sm text-zinc-500">Loading accounts...</div> : accounts.length === 0 ? <div className="card rounded-3xl border border-dashed border-[#dcece8] bg-[#f5fbf9] p-6 text-center" role="status" aria-live="polite"><div aria-hidden="true" className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-[#dff5ef] text-xl">🏦</div><h3 className="font-semibold text-[#16332f]">Belum ada akun</h3><p className="mx-auto mt-1 max-w-sm text-sm text-zinc-500">Buat akun pertama Anda untuk mulai melacak saldo dan memantau arus kas di satu tempat.</p></div> : <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2">{accounts.map((account) => <AccountCard key={account.id} account={account} actions={<><button type="button" onClick={() => editAccount(account)} className="rounded-full bg-[#e9f5f2] px-3 py-1.5 text-xs font-semibold text-[#087f6b]">Edit</button><button type="button" onClick={() => void archive(account)} className="rounded-full bg-zinc-100 px-3 py-1.5 text-xs font-semibold text-zinc-600">Archive</button></>} />)}</div>}
        </section>
