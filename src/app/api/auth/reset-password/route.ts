...
export async function POST(request: NextRequest) {
  try {
    ...
    const passwordValidation = validatePassword(body!.password as string);
    if (!passwordValidation.valid) {
      return NextResponse.json(validationErrorResponse(passwordValidation.errors), { status: 400 });
    }
